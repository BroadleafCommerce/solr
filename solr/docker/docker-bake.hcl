...
variable "FULLY_QUALIFIED_SOLR_TAG" {}

# The Docker Bake Context (e.g. the extracted solr distribution to copy)
variable "DOCKER_BAKE_CONTEXT" {}

# The Specific Dockerfile to be used
variable "SOLR_DOCKERFILE" {}

# The Base Image on which to base the SOLR image (e.g. eclipse-temurin-*)
variable "BASE_IMAGE" {}
...
target "solr" {
  context = "${DOCKER_BAKE_CONTEXT}"
  dockerfile = "${SOLR_DOCKERFILE}"
  platforms = get_target_platforms()
  tags = [ "${FULLY_QUALIFIED_SOLR_TAG}" ]
  args = {
